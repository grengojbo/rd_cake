...
class TemplatesController extends AppController {
    ...
    protected $base    = "Access Providers/Controllers/Templates/";
    protected $itemNote         = 'TemplateNote';
...
//------------------------------------------------------------------------
            ...
            if(isset($this->request->query['columns'])){
                ...
                foreach($columns as $c){
                    ...
                    if($column_name == 'notes'){
                        ...
                        $owner_id       = $i['Template']['user_id'];
                        $owner_tree     = $this->_find_parents($owner_id);
                        array_push($csv_line,$owner_tree); 
                    }else{
                        array_push($csv_line,$i['Template']["$column_name"]);  
                    }
                }
                fputcsv($fp, $csv_line,';','"');
            }
    ...
    public function index(){
        ...
        foreach($q_r as $i){
            ...
            array_push($items,array(
                ...
                'owner'                 => $owner_tree, 
                'available_to_siblings' => $i['Template']['available_to_siblings'],
       ...
       //__ Authentication + Authorization __
        ...
        if($user['group_name'] == Configure::read('group.ap')){  
            ...
            foreach($q_r as $i){
                $add_flag   = false;
                $owner_id   = $i['Template']['user_id'];
                $a_t_s      = $i['Template']['available_to_siblings'];
                $add_flag   = false;
    ...
    public function delete($id = null) {
	    ...
	    if(isset($this->data['id'])){   //Single item delete
            ...
            $owner_id   = $item['Template']['user_id'];
            if($owner_id != $user_id){
    ...
    public function note_index(){
        ...
        if(isset($this->request->query['for_id'])){
            $template_id = $this->request->query['for_id'];
            $q_r    = $this->{$this->modelClass}->{$this->itemNote}->find('all', 
                array(
                    'contain'       => array('Note'),
                    'conditions'    => array('TemplateNote.template_id' => $template_id)
                )
            );
            $this->User = ClassRegistry::init('User');
            foreach($q_r as $i){
    ...
    public function note_add(){
        ...
        if ($this->{$this->modelClass}->{$this->itemNote}->Note->save($this->request->data)) {
            $d                      = array();
            $d['TemplateNote']['template_id']   = $this->request->data['for_id'];
            $d['TemplateNote']['note_id'] = $this->{$this->modelClass}->TemplateNote->Note->id;
    ...
    public function note_del(){
	    ...
	    if(isset($this->data['id'])){   //Single item delete
            ...
            if($owner_id != $user_id){
                ...
                $this->{$this->modelClass}->TemplateNote->Note->id = $this->data['id'];
                $this->{$this->modelClass}->TemplateNote->Note->delete($this->data['id'],true);
            }
    ...
    function _build_common_query($user){
        ...
        $c['contain']   = array(
                            'TemplateNote'    => array('Note.note','Note.id','Note.available_to_siblings','Note.user_id'),
                            'User',
                            'TemplateAttribute'
                        );
        ...
        //====== AP FILTER =====
        //If the user is an AP; we need to add an extra clause to only show the Templates which he is allowed to see.
        if($user['group_name'] == Configure::read('group.ap')){  //AP
            ...
            foreach($this->parents as $i){
                ...
                if($i_id != $user_id){ //upstream
                    ...
                    array_push($tree_array,array('Template.user_id' => $i_id));
                }
            }
